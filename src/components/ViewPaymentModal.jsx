...
const Badge = ({ type }) => {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wide ${c.bg} ${c.text}`}
    >
  ...
  <div className="fixed inset-0 z-[99999] flex items-center justify-center bg-black/60 backdrop-blur-sm">
    <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm mx-4 p-6 text-center">
      ...
      <p className="text-sm text-slate-500 mb-1">
        This will permanently delete payment of{" "}
        <strong className="text-slate-800">{fmt(payment.amount)}</strong>
      </p>
...
const EditRow = ({ payment, banks, onSave, onCancel, saving }) => {
  ...
  return (
    <tr className="bg-indigo-50/60 border-b border-indigo-200">
      ...
      {/* Invoice / Client — read-only in edit */}
      <td className="px-3 py-2">
...
const exportToExcel = (rows) => {
  ...
  const summaryData = [
    ...
    [
      "Invoice Payments",
      rows.filter((r) => r.payment_type === "Invoice").length,
    ],
  ...
  XLSX.writeFile(
    wb,
    `Payments_Made_${new Date().toISOString().slice(0, 10)}.xlsx`
  );
};
...
const ViewPaymentModal = ({ isOpen, onClose, invoice }) => {
  ...
  const fetchPayments = useCallback(async () => {
    ...
    if (invoice?.id) q = q.eq("invoice_id", invoice.id);
      ...
      q = q.eq("invoice_number", invoice.invoice_number);
  ...
  const handleSave = async (id, form) => {
    ...
    try {
      ...
      // Also update the related software_entry amount + date
      ...
      if (currentPayment?.software_entry_id) {
        ...
        await supabase
          ...
          .update({
            ...
            remarks: payload.remarks || "Payment Made",
      ...
      // Note: software_entries update is best-effort; won't block on failure
        ...
        prev.map((p) =>
          p.id === id
            ? {
                ...
                ...payload,
                bank_master:
                  banks.find((b) => b.id === payload.bank_id) || p.bank_master,
              }
            : p
        )
      );
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      // 2. Reverse receivable if billable
      if (deleteTarget.is_billable && deleteTarget.invoice_id) {
        const { data: invData } = await supabase
          .from("invoices")
          .select("receivable_amount")
          .eq("id", deleteTarget.invoice_id)
          .single();

        if (invData) {
          const reversed =
            Number(invData.receivable_amount || 0) -
            Number(deleteTarget.amount || 0);

          await supabase
            .from("invoices")
            .update({
              receivable_amount: Math.max(0, reversed),
            })
            .eq("id", deleteTarget.invoice_id);
        }
      }
            ...
            <div className="relative mt-4 grid grid-cols-3 gap-2">
              {[
                ...
                },
                {
                  label: "Billable",
                  value:
                    filtered.filter((r) => r.is_billable).length + " items",
                  icon: CheckCircle,
                },
