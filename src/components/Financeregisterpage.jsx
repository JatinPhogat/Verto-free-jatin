...
function getPresetRange(preset) {
  ...
  if (preset === "this_month") {
    return { from: formatDateInput(new Date(now.getFullYear(), now.getMonth(), 1)), to: today };
  }
  ...
  if (preset === "30d") { const d = new Date(); d.setDate(d.getDate() - 29); return { from: formatDateInput(d), to: today }; }
  if (preset === "90d") { const d = new Date(); d.setDate(d.getDate() - 89); return { from: formatDateInput(d), to: today }; }
  if (preset === "ytd") { return { from: formatDateInput(new Date(now.getFullYear(), 0, 1)), to: today }; }
  return { from: "", to: "" };
}
...
function formatMonth(d) { if (!d) return "—"; return new Date(d).toLocaleDateString("en-IN", { month: "short", year: "numeric" }); }
    ...
    <div className="relative" ref={ref}>
      <button
        ...
        className={`flex items-center gap-2 px-3 py-2 rounded-xl text-xs font-semibold border transition-all ${
          value ? "bg-blue-50 border-blue-200 text-blue-700" : "bg-white border-gray-200 text-gray-600 hover:border-gray-300"
        }`}
      >
        ...
        {open && (
          ...
          >
            <button onClick={() => { onChange(null); setOpen(false); }}
              className={`w-full text-left px-3.5 py-2 text-xs font-semibold transition-colors ${!value ? "text-blue-600 bg-blue-50" : "text-gray-500 hover:bg-gray-50"}`}>
              {allLabel}
            </button>
            {options.map((opt) => (
              <button key={opt.value} onClick={() => { onChange(opt.value); setOpen(false); }}
                className={`w-full text-left px-3.5 py-2 text-xs font-medium flex items-center gap-2 transition-colors ${value === opt.value ? "text-blue-600 bg-blue-50" : "text-gray-600 hover:bg-gray-50"}`}>
  ...
  return (
    ...
    >
      ...
      <td className="px-3 py-3 text-sm text-gray-800 max-w-[220px]">
        <div className="truncate font-medium">{tx.client_name || tx.employee_name || tx.pay_head || "—"}</div>
      ...
      <td className="px-3 py-3 text-xs text-gray-500 whitespace-nowrap">{tx.invoice_number || tx.reference_no || "—"}</td>
      ...
      <td className="px-3 py-3 text-xs text-gray-500 whitespace-nowrap text-center">{tx.head_count ?? "—"}</td>
      <td className="px-3 py-3 text-xs text-gray-500 whitespace-nowrap">{tx.bank_name || "—"}</td>
      <td className="px-3 py-3 text-xs max-w-[180px]"><span className="text-gray-400 truncate block">{tx.remarks || "—"}</span></td>
      <td className="px-3 py-3 text-xs whitespace-nowrap">
        {tx.status ? (
          <span className="px-2 py-0.5 rounded-md bg-gray-100 text-gray-600 text-[11px] font-medium">{tx.status}</span>
        ) : "—"}
      </td>
      <td className="px-3 py-3 text-xs text-gray-400 whitespace-nowrap">
        {tx.created_by_email ? (
          <span className="flex items-center gap-1"><Mail className="w-3 h-3" />{tx.created_by_email}</span>
        ) : "—"}
      </td>
      <td className="pl-3 pr-4 py-3 text-sm font-bold text-gray-900 text-right whitespace-nowrap">{formatAmount(tx.amount)}</td>
    </motion.tr>
  );
}
...
export default function FinanceRegisterPage() {
  ...
  useEffect(() => { setPage(0); }, [search, type, entity, department, datePreset, customRange]);
  ...
  const fetchRows = useCallback(async (isAuto = false) => {
    ...
    try {
      ...
      const { data: totalsData } = await buildQuery().select("amount, transaction_type");
  ...
  const clearAllFilters = () => {
    setType(null); setEntity(null); setDepartment(null);
    setDatePreset("this_month"); setCustomRange(null);
  ...
  const byType = (types) =>
    allRowsForTotals
      .filter((r) => types.some((t) => r.transaction_type === t || r.transaction_type?.startsWith(t)))
      .reduce((sum, r) => sum + (Number(r.amount) || 0), 0);

  const payrollTotal   = byType(["Employee Payout", "OS Payout"]);
  const expenseTotal   = byType(["Payment Made", "Expense", "Statutory Payment", "CN / Bad Debt", "Interest/Penalty", "Bounce Back"]);
  ...
  const handleExport = () => {
    ...
    const csvRows = rows.map((r) => [
      formatDate(r.payment_date), r.transaction_type,
      r.client_name || r.employee_name || "", r.emp_code || "", r.employee_email || "",
      r.invoice_number || r.reference_no || "", r.department || "", r.entity || "",
      formatMonth(r.payout_month), r.head_count ?? "", r.pay_head || "", r.bank_name || "",
      `"${(r.remarks || "").replace(/"/g, '""')}"`,
      r.status || "", r.created_by_email || "", r.amount ?? "",
    ]);
    const csv  = [headers.join(","), ...csvRows.map((r) => r.join(","))].join("\n");
    const blob = new Blob([csv], { type: "text/csv;charset=utf-8;" });
    const url  = URL.createObjectURL(blob);
    const a    = document.createElement("a");
    a.href = url; a.download = `outgoing-register-${formatDateInput(new Date())}.csv`; a.click();
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-start justify-between flex-wrap gap-4">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h2 className="text-lg font-bold text-gray-900 tracking-tight">Outgoing Transaction Register</h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div className="bg-white rounded-3xl border border-gray-100 p-5" style={{ boxShadow: "0 1px 0 rgba(0,0,0,0.02), 0 8px 24px -12px rgba(220,38,38,0.10)" }}>
          ...
          <p className="text-2xl font-bold text-rose-600">{formatAmount(grandTotal)}</p>
          <p className="text-[11px] text-gray-400 mt-1">{allRowsForTotals.length.toLocaleString("en-IN")} entries in view</p>
        </div>

        {/* Payroll & OS */}
        <div className="bg-white rounded-3xl border border-gray-100 p-5" style={{ boxShadow: "0 1px 0 rgba(0,0,0,0.02), 0 8px 24px -12px rgba(8,145,178,0.10)" }}>
          <div className="flex items-center gap-2 mb-2">
            <div className="w-8 h-8 rounded-xl bg-cyan-50 border border-cyan-100 flex items-center justify-center">
              <Users className="w-4 h-4 text-cyan-600" />
            </div>
            <p className="text-xs font-semibold text-gray-500">Payroll & OS Payouts</p>
          </div>
          <p className="text-2xl font-bold text-cyan-700">{formatAmount(payrollTotal)}</p>
          <p className="text-[11px] text-gray-400 mt-1">Employee + OS Payouts</p>
        </div>

        {/* Payments, Expenses & Others */}
        <div className="bg-white rounded-3xl border border-gray-100 p-5" style={{ boxShadow: "0 1px 0 rgba(0,0,0,0.02), 0 8px 24px -12px rgba(217,119,6,0.10)" }}>
          <div className="flex items-center gap-2 mb-2">
            <div className="w-8 h-8 rounded-xl bg-amber-50 border border-amber-100 flex items-center justify-center">
              <Receipt className="w-4 h-4 text-amber-600" />
            </div>
            <p className="text-xs font-semibold text-gray-500">Payments, Expenses & Others</p>
          </div>
          <p className="text-2xl font-bold text-amber-700">{formatAmount(expenseTotal)}</p>
          <p className="text-[11px] text-gray-400 mt-1">Payments Made, Expenses, Statutory & more</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-3xl border border-gray-100 p-4 space-y-3"
        ...
        <div className="relative">
          ...
          />
          {searchInput && (
            <button onClick={() => setSearchInput("")} className="absolute right-3.5 top-1/2 -translate-y-1/2 text-gray-300 hover:text-gray-500">
              <X className="w-4 h-4" />
            </button>
          )}
        </div>
              ...
              {showDatePicker && (
                ...
                >
                  <div className="grid grid-cols-2 gap-1.5 mb-3">
                    {DATE_PRESETS.map((p) => (
                      <button key={p.id} onClick={() => { setDatePreset(p.id); setCustomRange(null); setShowDatePicker(false); }}
                  ...
                  <div className="border-t border-gray-100 pt-3">
                    ...
                    <div className="flex items-center gap-2">
                      <input type="date" defaultValue={dateRange.from}
                      ...
                      <input type="date" defaultValue={dateRange.to}
      ...
      <div className="bg-white rounded-3xl border border-gray-100 overflow-hidden"
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center">
            ...
            <p className="text-base font-semibold text-gray-700">No outgoing transactions found</p>
              ...
              <thead>
                <tr className="bg-gray-50 border-b border-gray-100">
                  {["Date","Type","Client / Employee","Invoice / Ref","Department","Entity",
                    "Payout Month","Head Count","Bank","Remarks","Status","Created By"].map((h) => (
      ...
      {!loading && totalCount > PAGE_SIZE && (
        <div className="flex items-center justify-between bg-white rounded-2xl border border-gray-100 px-4 py-3">
          ...
          <div className="flex items-center gap-1">
            ...
            <span className="px-3 text-xs font-semibold text-gray-600">Page {page + 1} of {totalPages}</span>
