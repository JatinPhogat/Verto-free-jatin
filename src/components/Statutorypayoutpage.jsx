...
import {
  RefreshCcw, Plus, Trash2, AlertCircle, CheckCircle2,
  FileCheck, Calendar, Building2, Hash, Search, X,
  ChevronDown, ChevronUp, Loader2, TrendingDown, AlertTriangle,
} from "lucide-react";
...
const Toast = ({ message, type, onDismiss }) => (
  <motion.div
    ...
    className={`fixed bottom-6 right-6 z-[9999] flex items-center gap-3 px-5 py-3.5 rounded-xl shadow-2xl text-white text-sm font-medium ${type === "success" ? "bg-emerald-600" : "bg-rose-600"}`}
  >
...
const ConfirmDialog = ({ onConfirm, onCancel, row }) => (
  ...
  >
    ...
    >
      ...
      <h3 className="text-lg font-bold text-gray-900 text-center">Delete Statutory Payout?</h3>
      <p className="text-sm text-gray-500 text-center mt-2">
        Removes linked bank & software entries and resyncs ERP.
        <br />
        <span className="font-semibold text-gray-700">
          {row?.type} · {row?.entity} · ₹ {Number(row?.total_paid || 0).toLocaleString("en-IN")}
        </span>
      </p>
      <div className="flex gap-3 mt-6">
        <button onClick={onCancel} className="flex-1 py-2.5 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 text-sm font-medium transition-colors">Cancel</button>
        <button onClick={onConfirm} className="flex-1 py-2.5 bg-rose-600 hover:bg-rose-700 text-white rounded-lg text-sm font-medium transition-colors">Yes, Delete</button>
      </div>
    </motion.div>
  </motion.div>
);
...
const StatCard = ({ label, value, sub, color, icon: Icon }) => (
  <div className={`rounded-xl p-4 border-2 ${color} bg-white`}>
    <div className="flex items-start justify-between">
      ...
      <div className="p-2 rounded-lg bg-gray-50"><Icon className="w-5 h-5 text-gray-400" /></div>
    </div>
  </div>
);
...
const StatutoryPayoutPage = () => {
  ...
  const fetchMeta = async () => {
    ...
    setBanks(bankData || []);
    const unique = [...new Set((entData || []).map((r) => r.entity).filter(Boolean))];
    setEntities(unique);
  };

  useEffect(() => { fetchRecords(); fetchMeta(); }, [fetchRecords]);

  // ── Real-time ─────────────────────────────────────────────────────────────────
  useEffect(() => {
    const channel = supabase
      .channel("statutory-live")
      .on("postgres_changes", { event: "*", schema: "public", table: "statutory_payments" }, async () => {
        await fetchRecords();
  ...
  const filtered = records.filter((r) => {
    ...
    const matchSearch = !q || [r.type, r.entity, r.bank_name, r.remarks].some((v) => v?.toLowerCase().includes(q));
    const matchType = filterType === "All" || r.type === filterType;
    const matchStatus = filterStatus === "All" || r.calculated_status === filterStatus;
    return matchSearch && matchType && matchStatus;
  });

  // ── Stats ─────────────────────────────────────────────────────────────────────
  const totalPaid = records.reduce((s, r) => s + Number(r.total_paid || 0), 0);
  const totalPending = records.reduce((s, r) => s + Number(r.pending_due || 0), 0);
  const totalPenalty = records.reduce((s, r) => s + Number(r.penalty_amount || 0), 0);
  const overdue = records.filter((r) => r.delay_days > 0).length;

  const typeOptions = ["All", "GST", "TDS", "EPF", "ESI", "LWF", "PF", "Income Tax", "Others"];
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-gradient-to-r from-cyan-600 to-blue-700 px-6 py-8 text-white">
        <div className="max-w-7xl mx-auto">
          <div className="flex items-center justify-between flex-wrap gap-4">
            <div>
              <h1 className="text-3xl font-black tracking-tight">STATUTORY PAYOUT</h1>
              <p className="text-cyan-200 text-sm mt-1">GST · TDS · EPF · ESI · LWF · PF · Income Tax</p>
            </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-6">
            <StatCard label="Total Records" value={records.length} sub="all time" color="border-cyan-200" icon={Hash} />
            <StatCard label="Total Paid" value={`₹ ${totalPaid.toLocaleString("en-IN")}`} sub="outflow" color="border-emerald-200" icon={FileCheck} />
            <StatCard label="Pending Due" value={`₹ ${totalPending.toLocaleString("en-IN")}`} sub="outstanding" color="border-rose-200" icon={TrendingDown} />
            <StatCard label="Overdue" value={overdue} sub={`₹ ${totalPenalty.toLocaleString("en-IN")} penalty`} color="border-amber-200" icon={AlertTriangle} />
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-6 py-6">
        ...
        <div className="flex items-center gap-3 mb-5 flex-wrap">
          <div className="relative flex-1 min-w-[200px] max-w-sm">
            ...
            <input type="text" placeholder="Search type, entity, bank..."
          ...
          <p className="text-sm text-gray-500 ml-auto">{filtered.length} of {records.length} records</p>
        </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
          {loading ? (
            <div className="flex items-center justify-center py-24 text-gray-400">
              <Loader2 className="w-6 h-6 animate-spin mr-3" /> Loading records...
            </div>
          ) : filtered.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-24 text-gray-400">
              <FileCheck className="w-10 h-10 mb-3 opacity-30" />
              <p className="text-sm font-medium">No statutory payout records found</p>
              <button onClick={() => setIsModalOpen(true)}
              ...
              <table className="w-full text-sm">
                  ...
                  <tr className="bg-gray-50 border-b border-gray-200">
                    {[
                      ...
                      <th key={label} onClick={() => field && handleSort(field)}
                        className={`px-4 py-3 text-left text-xs font-bold uppercase tracking-wider text-gray-500 whitespace-nowrap ${field ? "cursor-pointer hover:text-gray-800 select-none" : ""}`}>
                  ...
                  <AnimatePresence>
                    {filtered.map((row, idx) => (
                      <motion.tr key={row.id}
                        ...
                        className={`hover:bg-cyan-50/40 transition-colors ${deletingId === row.id ? "opacity-40 pointer-events-none" : ""}`}
                      >
                        <td className="px-4 py-3 text-gray-400 font-mono text-xs">{idx + 1}</td>
                        <td className="px-4 py-3">
                          <span className="font-bold text-cyan-700 text-xs bg-cyan-50 px-2 py-1 rounded">{row.type}</span>
                        </td>
                        <td className="px-4 py-3 text-gray-700 text-xs font-medium">{row.entity || "—"}</td>
                        <td className="px-4 py-3 text-gray-600 text-xs whitespace-nowrap">
                          {row.month ? new Date(row.month).toLocaleDateString("en-IN", { month: "short", year: "numeric" }) : "—"}
                        </td>
                        <td className="px-4 py-3 text-gray-600 text-xs whitespace-nowrap">{row.bank_name || "—"}</td>
                        ...
                        <td className="px-4 py-3">
                          <span className={`font-bold text-xs whitespace-nowrap ${Number(row.pending_due) > 0 ? "text-rose-600" : "text-gray-400"}`}>
                            ₹ {Number(row.pending_due || 0).toLocaleString("en-IN")}
                          </span>
                        </td>
                        <td className="px-4 py-3"><StatusBadge status={row.calculated_status} /></td>
                        <td className="px-4 py-3 text-xs">
                          {row.delay_days > 0
                            ? <span className="text-rose-600 font-semibold">{row.delay_days}d late</span>
                            : <span className="text-gray-400">—</span>}
                        </td>
                        <td className="px-4 py-3 text-xs">
                          {row.penalty
                            ? <span className="text-amber-600 font-semibold">₹ {Number(row.penalty_amount || 0).toLocaleString("en-IN")}</span>
                            : <span className="text-gray-400">—</span>}
                        </td>
                        <td className="px-4 py-3">
                          {deletingId === row.id ? (
                ...
                <tfoot>
                  <tr className="bg-cyan-50 border-t-2 border-cyan-200">
                    <td colSpan={5} className="px-4 py-3 text-xs font-bold uppercase tracking-wider text-cyan-800">
                      Total ({filtered.length} records)
                    </td>
                    <td className="px-4 py-3 font-black text-gray-700 text-xs whitespace-nowrap">
                      ₹ {filtered.reduce((s, r) => s + Number(r.total_due || 0), 0).toLocaleString("en-IN")}
                    </td>
                    <td className="px-4 py-3 font-black text-emerald-700 text-xs whitespace-nowrap">
                      ₹ {filtered.reduce((s, r) => s + Number(r.total_paid || 0), 0).toLocaleString("en-IN")}
                    </td>
                    <td className="px-4 py-3 font-black text-rose-700 text-xs whitespace-nowrap">
                      ₹ {filtered.reduce((s, r) => s + Number(r.pending_due || 0), 0).toLocaleString("en-IN")}
                    </td>
                    <td colSpan={4} />
                  </tr>
                </tfoot>
      ...
      {/* Dialogs */}
      <AnimatePresence>
        {confirmRow && <ConfirmDialog onConfirm={handleDelete} onCancel={() => setConfirmRow(null)} row={confirmRow} />}
      </AnimatePresence>
      <AnimatePresence>
      ...
      <AddStatutoryPayoutModal
        isOpen={isModalOpen}
        onClose={() => { setIsModalOpen(false); fetchRecords(); }}
        banks={banks}
        entities={entities}
      />
    </div>
