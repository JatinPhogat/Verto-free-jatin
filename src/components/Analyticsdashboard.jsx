...
import {
  ...
  Wallet, Building2, BarChart2, Search, FileX,
} from "lucide-react";
...
// ─── internal_team uses short codes; map to full dept names ──────────────────
const TEAM_DEPT_MAP = {
  OS:     "Operations",
  Rec:    "Recruitment",
...
// ✅ impact_month / payout_month stored as full date "2026-04-01" → slice to "2026-04"
const toYYYYMM = (d) => (d || "").slice(0, 7);

const fmtMonth = (yyyymm) => {
  if (!yyyymm) return "";
  return new Date(yyyymm + "-01").toLocaleDateString("en-IN", { month: "short", year: "2-digit" });
};
...
const CustomTooltip = ({ active, payload, label }) => {
  ...
  return (
    <div className="bg-white border border-gray-100 rounded-xl shadow-xl p-3 text-xs min-w-[150px]">
...
export default function AnalyticsDashboard() {
    ...
    try {
      const [
        ...
        supabase.from("invoices").select(`
          ...
          employee_count, pay_head, status,
          client_id, department_id, entity_id,
        ...
        `),
        // ✅ os_payouts: department_id is always NULL — will fallback to invoice dept
        ...
        `),
        // ✅ credit_note_bad_debt: fetch with invoice join for dept/client
        supabase.from("credit_note_bad_debt").select(`
          ...
          er_pf, ee_pf, er_esic, ee_esic, lwf_cn, pt_cn,
          invoices (
            department_id,
            clients_master!invoices_client_id_fkey ( client_name )
          )
  ...
  // ── Lookup maps ───────────────────────────────────────────────────────────
  // dept_id → dept_name (for invoices which have no FK join)
  const deptById = useMemo(() => {
    const map = {};
    departments.forEach((d) => { map[d.id] = d.dept_name; });
    return map;
  }, [departments]);

  const entityById = useMemo(() => {
    const map = {};
    entities.forEach((e) => { map[e.id] = e.entity_name; });
    return map;
  }, [entities]);

  // emp_code → internal_team dept (short code → full name)
  // Used as FALLBACK when employee_expense_payouts.department_id is null
  const empDeptByCode = useMemo(() => {
    const map = {};
    team.forEach((t) => {
      if (t.emp_code) map[t.emp_code] = TEAM_DEPT_MAP[t.department] || t.department || "Unknown";
    });
    return map;
  }, [team]);
  ...
  const flatInvoices = useMemo(() => invoices.map((i) => ({
    ...
    // ✅ Dept: manual lookup since invoices has no dept FK join
  ...
  // ── Flatten OS payouts ────────────────────────────────────────────────────
  // ✅ DEPT FALLBACK: os_payouts.department_id is NULL → use invoice's dept
  const flatOs = useMemo(() => osPayouts.map((o) => {
    const invDeptId = o.invoices?.department_id;
    const deptName  = deptById[o.department_id] || deptById[invDeptId] || "Unknown";
    return {
      ...o,
      client_name: o.clients_master?.client_name  || "Unknown",
      dept_name:   deptName,
      entity_name: o.entity_master?.entity_name || entityById[o.entity_id] || "Unknown",
      // ✅ payout_month is NULL → use payment_date for month grouping
      effective_month: toYYYYMM(o.payout_month) || toYYYYMM(o.payment_date),
    };
  }), [osPayouts, deptById, entityById]);

  // ── Flatten credit notes ──────────────────────────────────────────────────
  // ✅ CN: get dept & client from joined invoice
  const flatCN = useMemo(() => creditNotes.map((cn) => ({
    ...cn,
  ...
  // ── Flatten salaries ──────────────────────────────────────────────────────
  // ✅ EMP DEPT FALLBACK: if department_id is null → use internal_team dept via emp_code
  const flatSalaries = useMemo(() => salaries.map((s) => {
    const deptFromPayroll = s.departments_master?.dept_name;
    const deptFromTeam    = empDeptByCode[s.emp_code];
    return {
      ...s,
      dept_name:   deptFromPayroll || deptFromTeam || "Unknown",
  ...
  const filteredInvoices = useMemo(() => flatInvoices.filter((i) => {
    ...
    if (filters.status       && i.status?.toUpperCase() !== filters.status?.toUpperCase()) return false;
  ...
  // ✅ CN filtered — by issue_date and dept/client from invoice
  const filteredCN = useMemo(() => flatCN.filter((cn) => {
  ...
  const filteredTeam = useMemo(() => team.filter((t) => {
    const fullDept = TEAM_DEPT_MAP[t.department] || t.department;
    if (filters.department && fullDept !== filters.department) return false;
  ...
  const kpis = useMemo(() => {
    ...
    // ✅ subtract CN verto fee
    const cnVertoDeducted   = filteredCN.reduce((s, cn) => s + Number(cn.verto_fee_cn || 0), 0);
    const netVertoFee       = totalVerto - cnVertoDeducted;
    ...
    const totalCN           = filteredCN.reduce((s, cn)      => s + Number(cn.amount           || 0), 0);
    ...
    return {
      totalInvoiceValue, netVertoFee, totalOutstanding, totalReceived,
      totalOsPayout, totalSalary, totalCN, activeEmployees,
      collectionPct, bankInflow, bankOutflow, bankNet: bankInflow - bankOutflow,
    };
  }, [filteredInvoices, filteredPayments, filteredOs, filteredCN, filteredSalaries, filteredTeam, filteredBank]);

  // ── Chart 1: Invoice by month (with CN deductions shown) ─────────────────
  const invoiceByMonth = useMemo(() => {
    const map = {};
    filteredInvoices.forEach((i) => {
      const m = toYYYYMM(i.impact_month) || toYYYYMM(i.invoice_date);
      if (!m) return;
      if (!map[m]) map[m] = { month: m, invoiceValue: 0, vertoFee: 0, gst: 0, tds: 0, cnAmount: 0, netVerto: 0 };
    ...
    // ✅ Subtract CN amounts per month
    filteredCN.forEach((cn) => {
      const m = toYYYYMM(cn.issue_date);
      if (!m || !map[m]) return;
      map[m].cnAmount  += Number(cn.amount       || 0);
      map[m].netVerto  -= Number(cn.verto_fee_cn || 0);
      map[m].gst       -= Number(cn.gst_cn       || 0);
      map[m].tds       -= Number(cn.tds_cn       || 0);
    });
    return Object.values(map)
      ...
      .map((d) => ({
        ...
        netVerto: d.vertoFee + d.netVerto, // vertoFee - cn deductions
      }));
  }, [filteredInvoices, filteredCN]);
  ...
  // ── Chart 3: Revenue by client (with CN deducted) ────────────────────────
  const revenueByClient = useMemo(() => {
    const map = {};
    filteredInvoices.forEach((i) => {
      const c = i.client_name;
      if (!map[c]) map[c] = { client: c, "Invoice Value": 0, "Verto Fee": 0, "Received": 0, "CN Deducted": 0 };
  ...
  // ── Chart 6: GST & TDS by month (NET after CN) ───────────────────────────
  // ✅ Already computed inside invoiceByMonth above — reuse those fields
  // invoiceByMonth has .gst (net) and .tds (net) after CN subtraction

  // ── Chart 7: Invoice breakdown ────────────────────────────────────────────
  ...
  const collectionPerInvoice = useMemo(() => {
    ...
    return filteredInvoices.map((i) => {
      ...
      const pct = Number(i.invoice_value) > 0
        ? Math.min(100, Math.round((received / Number(i.invoice_value)) * 100)) : 0;
      return {
        invoice: i.invoice_number,
        "Invoice Value":   Number(i.invoice_value || 0),
  ...
  // ── Chart 11: CN by month ─────────────────────────────────────────────────
  const cnByMonth = useMemo(() => {
    const map = {};
    filteredCN.forEach((cn) => {
      const m = toYYYYMM(cn.issue_date); if (!m) return;
      if (!map[m]) map[m] = { month: m, total: 0, vertoFee: 0, gst: 0, pay: 0 };
      map[m].total   += Number(cn.amount       || 0);
      map[m].vertoFee += Number(cn.verto_fee_cn || 0);
      map[m].gst     += Number(cn.gst_cn       || 0);
      map[m].pay     += Number(cn.pay_cn       || 0);
    });
    return Object.values(map)
      .sort((a, b) => a.month.localeCompare(b.month))
  ...
  // ── Chart 12: CN by client ────────────────────────────────────────────────
  const cnByClient = useMemo(() => {
    const map = {};
    filteredCN.forEach((cn) => {
      const c = cn.client_name;
      if (!map[c]) map[c] = { client: c, total: 0, vertoFee: 0, gst: 0 };
      map[c].total    += Number(cn.amount       || 0);
      map[c].vertoFee += Number(cn.verto_fee_cn || 0);
      map[c].gst      += Number(cn.gst_cn       || 0);
    });
    return Object.values(map).sort((a, b) => b.total - a.total);
  }, [filteredCN]);

  // ── Chart 13: OS by month ─────────────────────────────────────────────────
  // ✅ payout_month is NULL everywhere — use effective_month (payment_date fallback)
  const osByMonth = useMemo(() => {
    const map = {};
    filteredOs.forEach((o) => {
      const m = o.effective_month; if (!m) return;
  ...
  // ── Chart 15: OS by dept (using invoice dept fallback) ───────────────────
  const osByDept = useMemo(() => {
    const map = {};
    filteredOs.forEach((o) => {
      const d = o.dept_name;
      if (!map[d]) map[d] = { dept: d, amountPaid: 0, employeeCount: 0 };
      map[d].amountPaid    += Number(o.amount_paid    || 0);
      map[d].employeeCount += Number(o.employee_count || 0);
    });
    return Object.values(map).sort((a, b) => b.amountPaid - a.amountPaid);
  }, [filteredOs]);

  // ── Chart 16: Billable vs non-billable OS ────────────────────────────────
  ...
  // ── Chart 17: Salary by month ─────────────────────────────────────────────
  const salaryByMonth = useMemo(() => {
    const map = {};
    filteredSalaries.forEach((s) => {
      const m = toYYYYMM(s.month_of_pay); if (!m) return;
  ...
  // ── Chart 18: Salary by dept (with fallback dept) ────────────────────────
  ...
  const bankFlow = useMemo(() => {
    ...
    filteredBank.forEach((b) => {
      ...
      if (isBankInflow(b)) map[d].Inflow += amt; else map[d].Outflow += amt;
    });
  ...
  const softwareFlow = useMemo(() => {
    ...
    filteredSoftware.forEach((s) => {
      ...
      if (isSoftwareInflow(s)) map[d].Inflow += amt; else map[d].Outflow += amt;
    });
    return Object.values(map).sort((a, b) => a.date.localeCompare(b.date)).map((d) => ({ ...d, date: fmtDate(d.date) }));
  }, [filteredSoftware]);
  ...
  const impactMonthOptions = [...new Set(invoices.map((i) => toYYYYMM(i.impact_month)).filter(Boolean))].sort().reverse().map((m) => ({ value: m, label: fmtMonth(m) }));
  ...
  return (
    <div className="space-y-6 pb-10">
      ...
      {filtersOpen && (
        <div className="bg-white border border-gray-100 rounded-2xl shadow-sm p-5">
          <div className="flex items-center justify-between mb-4">
            <span className="text-xs font-black text-gray-500 uppercase tracking-wider">Filters</span>
            {activeFilterCount > 0 && (
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <KpiCard label="Net Verto Fee"       value={fmt(kpis.netVertoFee)}        sub="After CN deductions"                                          icon={TrendingUp}  color={P.indigo} />
        ...
        <KpiCard label="Outstanding"         value={fmt(kpis.totalOutstanding)}   sub={outstandingByClient.length > 0 ? `${outstandingByClient.length} clients` : "All cleared ✓"} icon={Activity} color={kpis.totalOutstanding > 0 ? P.amber : P.emerald} />
        <KpiCard label="Credit Notes Total"  value={fmt(kpis.totalCN)}            sub={`${filteredCN.length} CN entries`}                            icon={FileX}       color={P.rose} />
        ...
        <KpiCard label="Active Employees"    value={kpis.activeEmployees}         sub="Internal team"                                                icon={Users}       color={P.teal} />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          {invoiceByMonth.length === 0 ? <Empty /> : (
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={invoiceByMonth} margin={{ top: 5, right: 10, left: 0, bottom: 5 }}>
                ...
                <Bar dataKey="netVerto"     name="Net Verto Fee" fill={P.indigo} radius={[4,4,0,0]} />
              </BarChart>
            </ResponsiveContainer>
          )}
        </ChartCard>
        ...
        <ChartCard title="Revenue by Client" subtitle="Invoice value, verto fee, received & CN deducted">
          {revenueByClient.length === 0 ? <Empty /> : (
            <ResponsiveContainer width="100%" height={Math.max(220, revenueByClient.length * 60)}>
              <BarChart data={revenueByClient} layout="vertical" margin={{ top: 5, right: 50, left: 10, bottom: 5 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" horizontal={false} />
                <XAxis type="number" tick={{ fontSize: 10 }} tickFormatter={fmt} />
                <YAxis dataKey="client" type="category" tick={{ fontSize: 9 }} width={140} />
                <Tooltip content={<CustomTooltip />} />
                <Legend wrapperStyle={{ fontSize: 10 }} />
                ...
                <Bar dataKey="CN Deducted"   fill={P.rose}    radius={[0,4,4,0]} />
              </BarChart>
            </ResponsiveContainer>
          )}
        </ChartCard>

        <ChartCard title="Revenue by Department" subtitle="Invoice value & CN deducted per department">
          {revenueByDept.length === 0 ? <Empty /> : (
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={revenueByDept} margin={{ top: 5, right: 10, left: 0, bottom: 5 }}>
                ...
                <Bar dataKey="cnDeducted"   name="CN Deducted"   fill={P.rose}   radius={[4,4,0,0]} />
              </BarChart>
            </ResponsiveContainer>
          )}
        </ChartCard>
        ...
        {/* ✅ GST & TDS NET after CN deductions */}
        <ChartCard title="GST & TDS by Month (Net after CN)" subtitle="Tax components from invoices minus credit note adjustments">
          {invoiceByMonth.length === 0 ? <Empty /> : (
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={invoiceByMonth} margin={{ top: 5, right: 10, left: 0, bottom: 5 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" />
                ...
                <Tooltip content={<CustomTooltip />} />
                <Legend wrapperStyle={{ fontSize: 10 }} />
                <Bar dataKey="gst" name="Net GST" fill={P.amber} radius={[4,4,0,0]} />
                <Bar dataKey="tds" name="Net TDS" fill={P.rose}  radius={[4,4,0,0]} />
              </BarChart>
            </ResponsiveContainer>
          )}
        </ChartCard>
      ...
      <SectionHeader icon={FileX} title="Credit Notes & Bad Debt" color={P.rose} />
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">

        <ChartCard title="CN Amount by Month" subtitle="Total credit note value raised per month">
          {cnByMonth.length === 0 ? <Empty msg="No credit notes in selected range" /> : (
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={cnByMonth} margin={{ top: 5, right: 10, left: 0, bottom: 5 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" />
                <XAxis dataKey="month" tick={{ fontSize: 10 }} />
                <YAxis tick={{ fontSize: 10 }} tickFormatter={fmt} />
                <Tooltip content={<CustomTooltip />} />
                <Legend wrapperStyle={{ fontSize: 10 }} />
                <Bar dataKey="total"    name="Total CN"   fill={P.rose}  radius={[4,4,0,0]} />
                <Bar dataKey="vertoFee" name="Verto Fee CN" fill={P.orange} radius={[4,4,0,0]} />
                <Bar dataKey="gst"      name="GST CN"     fill={P.amber} radius={[4,4,0,0]} />
              </BarChart>
            </ResponsiveContainer>
          )}
        </ChartCard>
        ...
        <ChartCard title="CN by Client" subtitle="Credit note impact per client">
          {cnByClient.length === 0 ? <Empty msg="No credit notes in selected range" /> : (
            <ResponsiveContainer width="100%" height={Math.max(220, cnByClient.length * 55)}>
              <BarChart data={cnByClient} layout="vertical" margin={{ top: 5, right: 50, left: 10, bottom: 5 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" horizontal={false} />
                <XAxis type="number" tick={{ fontSize: 10 }} tickFormatter={fmt} />
                <YAxis dataKey="client" type="category" tick={{ fontSize: 9 }} width={140} />
                <Tooltip content={<CustomTooltip />} />
                <Legend wrapperStyle={{ fontSize: 10 }} />
                <Bar dataKey="total"    name="Total CN"     fill={P.rose}   radius={[0,4,4,0]} />
                <Bar dataKey="vertoFee" name="Verto Fee CN" fill={P.orange} radius={[0,4,4,0]} />
                <Bar dataKey="gst"      name="GST CN"       fill={P.amber}  radius={[0,4,4,0]} />
              </BarChart>
            </ResponsiveContainer>
          )}
        </ChartCard>
          ...
          {paymentsTrend.length === 0 ? <Empty /> : (
            <ResponsiveContainer width="100%" height={220}>
              <AreaChart data={paymentsTrend} margin={{ top: 5, right: 10, left: 0, bottom: 5 }}>
                <defs>
                  <linearGradient id="rcvG" x1="0" y1="0" x2="0" y2="1">
                ...
                <YAxis tick={{ fontSize: 10 }} tickFormatter={fmt} />
                <Tooltip content={<CustomTooltip />} />
                <Area type="monotone" dataKey="received" name="Received" stroke={P.emerald} fill="url(#rcvG)" strokeWidth={2} dot={{ r: 4 }} />
              </AreaChart>
            </ResponsiveContainer>
          )}
        </ChartCard>
      ...
      <SectionHeader icon={Wallet} title="OS Payouts" color={P.orange} />
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">

        {/* ✅ Uses payment_date fallback since payout_month is NULL */}
        <ChartCard title="OS Payout by Month" subtitle="Amount paid — grouped by payment date (payout_month not set)">
        ...
        {/* ✅ Employee Count by Month — same fix */}
        ...
        <ChartCard title="OS Payout by Client" subtitle="Amount paid per client">
          {osByClient.length === 0 ? <Empty /> : (
            <ResponsiveContainer width="100%" height={Math.max(220, osByClient.length * 60)}>
        ...
        {/* ✅ OS by Dept — uses invoice dept as fallback */}
        <ChartCard title="OS Payout by Department" subtitle="Dept from invoice (os_payouts.department_id not set)">
          {osByDept.length === 0 ? <Empty /> : (
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={osByDept} margin={{ top: 5, right: 10, left: 0, bottom: 5 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" />
                <XAxis dataKey="dept" tick={{ fontSize: 10 }} />
                <YAxis tick={{ fontSize: 10 }} tickFormatter={fmt} />
                <Tooltip content={<CustomTooltip />} />
                <Bar dataKey="amountPaid" name="Amount Paid" fill={P.orange} radius={[4,4,0,0]}>
                  <LabelList dataKey="amountPaid" position="top" formatter={fmt} style={{ fontSize: 9, fill: "#f97316" }} />
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          )}
      ...
      {/* ══ SECTION 5: SALARY ══ */}
      <SectionHeader icon={CreditCard} title="Internal Salary" color={P.violet} />
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        {/* ✅ Uses internal_team dept fallback when department_id is null */}
        <ChartCard title="Salary by Department" subtitle="Net salary per dept — falls back to internal_team dept via emp_code">
          {salaryByDept.length === 0 ? <Empty /> : (
            <ResponsiveContainer width="100%" height={Math.max(220, salaryByDept.length * 45)}>
              <BarChart data={salaryByDept} layout="vertical" margin={{ top: 5, right: 50, left: 10, bottom: 5 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" horizontal={false} />
                <XAxis type="number" tick={{ fontSize: 10 }} tickFormatter={fmt} />
                <YAxis dataKey="dept" type="category" tick={{ fontSize: 9 }} width={100} />
                <Tooltip content={<CustomTooltip />} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <ChartCard title="Employee Count by Department" subtitle="OS, Rec, BD, Accts, Temp, Common">
        ...
        <ChartCard title="Employee Location Distribution" subtitle="Top cities by headcount">
          {locationDist.length === 0 ? <Empty msg="No location data" /> : (
        ...
        <ChartCard title="Designation Distribution" subtitle="Top 12 designations">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <ChartCard title="Bank Inflow vs Outflow" subtitle="Daily bank movements">
          {bankFlow.length === 0 ? <Empty /> : (
            <ResponsiveContainer width="100%" height={220}>
              <AreaChart data={bankFlow} margin={{ top: 5, right: 10, left: 0, bottom: 5 }}>
                <defs>
                  <linearGradient id="bIn"  x1="0" y1="0" x2="0" y2="1"><stop offset="5%"  stopColor={P.emerald} stopOpacity={0.25} /><stop offset="95%" stopColor={P.emerald} stopOpacity={0} /></linearGradient>
                  <linearGradient id="bOut" x1="0" y1="0" x2="0" y2="1"><stop offset="5%"  stopColor={P.rose}    stopOpacity={0.2}  /><stop offset="95%" stopColor={P.rose}    stopOpacity={0} /></linearGradient>
                </defs>
                <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" />
          ...
          {softwareFlow.length === 0 ? <Empty /> : (
            <ResponsiveContainer width="100%" height={220}>
              <AreaChart data={softwareFlow} margin={{ top: 5, right: 10, left: 0, bottom: 5 }}>
                <defs>
                  <linearGradient id="swIn"  x1="0" y1="0" x2="0" y2="1"><stop offset="5%"  stopColor={P.sky}   stopOpacity={0.25} /><stop offset="95%" stopColor={P.sky}   stopOpacity={0} /></linearGradient>
                  <linearGradient id="swOut" x1="0" y1="0" x2="0" y2="1"><stop offset="5%"  stopColor={P.amber} stopOpacity={0.2}  /><stop offset="95%" stopColor={P.amber} stopOpacity={0} /></linearGradient>
                </defs>
                <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" />
                <XAxis dataKey="date" tick={{ fontSize: 9 }} />
                <YAxis tick={{ fontSize: 10 }} tickFormatter={fmt} />
